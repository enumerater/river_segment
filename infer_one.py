...
def get_default_prompt(img_size=1024):
    """Default prompt: full-image bounding box in absolute pixel coords."""
    return torch.tensor([[0, 0, img_size, img_size]], dtype=torch.float32)


def parse_prompt(prompt_str):
    """Parse comma-separated prompt string into tensor (N, 4)."""
...
def load_image(img_path, img_size):
    """Load and transform a single image (no gt pair needed)."""
    img = Image.open(img_path).convert('RGB')
    orig_size = img.size  # (W, H)
    img = TF.resize(img, [img_size, img_size])
    img = TF.to_tensor(img)
    img = TF.normalize(img, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225))
    return img.unsqueeze(0), orig_size  # (1, 3, H, W)
...
def predict(net, image, prompt, multimask_output, img_size):
    """Run inference and return the prediction mask."""
    with torch.no_grad():
...
def visualize(image_tensor, pred_mask, probs, orig_size, save_path):
    """Save a side-by-side visualization of the input and prediction."""
...
def main():
    ...
    parser.add_argument('--prompt', type=str, default=None,
                        help='Bounding box prompt as comma-separated values (N*4). Default: full-image box')
