...
class LoRA_Sam(nn.Module):
    ...
    def forward(self, batched_input,prompt,multimask_output=False, image_size=512):
        return self.sam(batched_input, prompt,multimask_output, image_size)
...
if __name__ == "__main__":
    ...
    lora_sam.cuda()
    lora_sam.load_lora_parameters(
        r"./best_model.pth")
    print(lora_sam.sam.image_encoder(torch.rand(size=(1, 3, 512, 512)).cuda()).shape)
    outputs = lora_sam(torch.rand(size=(1, 3, 512, 512)).cuda(), False, 512)
